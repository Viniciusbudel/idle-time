...
- [x] Fix `MegaChamberCard displays critical stats` test failure in `test/presentation/ui/widgets/mega_chamber_card_test.dart`
- [x] The failure stems from the removal of `worker.level` — update test fixtures to use the new artifact-based model
...
- [x] Create `test/domain/worker_artifact_test.dart`:
  - [x] Test `WorkerArtifact.generate()` for all 5 rarities
  - [x] Test `Worker.currentProduction` with 0, 1, and 5 artifacts equipped
  - [x] Test era-match bonus: artifact era == worker era → +10%
  - [x] Test `GameStateNotifier.equipArtifact` adds to worker, removes from inventory
  - [x] Test `GameStateNotifier.unequipArtifact` does the reverse
  - [x] Test inventory cap (100 items) rejects new artifacts
...
- [x] Create `lib/domain/entities/artifact_name_registry.dart` with a pool of lore-appropriate names per rarity:
...
- [x] Wire `WorkerArtifact.generate()` to use the registry instead of hardcoded names
...
- [x] Add rarity filter chips at top of `ArtifactInventoryDialog` (ALL / ★ / ★★ / ★★★ / ★★★★)
- [x] Show stats tooltip on long-press: `+X base power`, `+Y% production`, era bonus flag
- [x] Show slot-full warning in the EQUIP button when worker has 5 artifacts (currently just disables)
- [x] Animate the EQUIP action — brief green flash on the slot in `WorkerDetailDialog` after equip
...
- [x] Create `lib/presentation/game/components/temporal_anomaly_component.dart` as a `PositionComponent` (Flame):
...
- [x] Create `lib/domain/usecases/roll_artifact_drop_usecase.dart`:
...
- [x] Add a `_anomalySpawnTimer` to `TimeFactoryGame`:
...
- [x] Wire `addArtifactToInventory` result → show a brief toast/banner notification
...
- [x] Create `lib/presentation/ui/atoms/artifact_drop_banner.dart`:
  - Slides in from top with rarity color glow
  - Shows artifact name + rarity tier + icon
  - Auto-dismisses after 3 seconds
- [x] Trigger banner from `FactoryScreen` when `addArtifactToInventory` succeeds
...
- [x] Show a "Total Power" stat bar in `WorkerDetailDialog`:
  - Displays `worker.currentProduction` (with artifacts applied)
  - Shows breakdown: Base + Artifact Bonuses + Era Multiplier
- [x] Slot icons: use unique `IconData` per artifact type (gear, lens, coil, crystal, orb) instead of `auto_awesome`
...
- [x] Run `dart analyze` → fix all warnings/errors introduced by the artifact system
- [x] Remove unused imports from modified files
- [x] Ensure `const` is used where possible in artifact/widget constructors
...
- [x] All 8 wins completed
- [x] `flutter test` → all tests pass
- [x] `dart analyze` → 0 errors
- [x] Player can: receive artifact drop from anomaly → open inventory → equip to worker → see production increase
